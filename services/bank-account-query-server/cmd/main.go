// Файл bank-account-query-server/cmd/main.go содержит реализацию пакета main.
package main
...
import (
	...
	"github.com/rusgainew/kkm-project-mks/bank-account-query-server/internal/infrastructure/config"
	apphealth "github.com/rusgainew/kkm-project-mks/bank-account-query-server/internal/infrastructure/health"
	"github.com/rusgainew/kkm-project-mks/bank-account-query-server/internal/infrastructure/middleware"
	"github.com/rusgainew/kkm-project-mks/bank-account-query-server/internal/infrastructure/observability"
	"github.com/rusgainew/kkm-project-mks/bank-account-query-server/internal/infrastructure/repository"
	"github.com/rusgainew/kkm-project-mks/bank-account-query-server/internal/interfaces/grpc/handlers"
...
func main() {
	...
	tp, err := observability.InitTracer("bank-account-query-service", cfg.JaegerEndpoint)
	if err != nil {
	...
	repo := repository.NewInMemoryBankAccountRepository(logger)
	logger.Info("In-memory repository initialized")

	// Initialize metrics
	metrics := observability.NewMetricsCollector("bank_account_query", "service")
	...
	healthServer.SetServingStatus("api.BankAccountQueryService", grpc_health_v1.HealthCheckResponse_SERVING)

	// Register handler with repository, metrics, and cache
	handler := handlers.NewBankAccountQueryHandler(logger, repo, metrics, redisCache)
	pb.RegisterBankAccountQueryServiceServer(grpcServer, handler)

	// Initialize health check manager
	healthManager := apphealth.NewManager(logger)

	// Get the concrete repository type for health checks
	inMemoryRepo, ok := repo.(*repository.InMemoryBankAccountRepository)
	if ok {
	...
	logger.Info("Bank Account Query Server starting", zap.Int("grpc_port", cfg.GRPCPort))
	...
	logger.Info("Bank Account Query Server stopped")
}
