// Файл catalog-query-server/internal/infrastructure/config/config.go содержит реализацию пакета config.
package config
...
func Load() *Config {
	cfg := &Config{
		GRPCPort:        loadPortEnv("GRPC_PORT", 50063),
		HealthPort:      loadPortEnv("METRICS_PORT", 9106),
