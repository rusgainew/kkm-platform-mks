...
func (c *RabbitMQConsumer) republishWithRetry(msg amqp.Delivery, retryCount int, delay time.Duration) {
...
func (c *RabbitMQConsumer) sendToDLQ(msg amqp.Delivery, retryCount int) error {
	headers := amqp.Table{}
	if msg.Headers != nil {
		for k, v := range msg.Headers {
			headers[k] = v
