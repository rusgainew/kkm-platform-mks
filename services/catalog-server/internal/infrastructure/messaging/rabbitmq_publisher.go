// Файл catalog-server/internal/infrastructure/messaging/rabbitmq_publisher.go содержит реализацию пакета messaging.
package messaging
...
func NewRabbitMQPublisher(url string, logger *zap.Logger) (*RabbitMQPublisher, error) {
	...
	// Объявление exchange для каталога
	...
	logger.Info("RabbitMQ publisher connected", zap.String("exchange", "catalog.events"))
...
func getRoutingKey(event interface{}) string {
	switch event.(type) {
	case interface{ ToJSON() ([]byte, error) }:
		// Извлекаем тип события из структуры
