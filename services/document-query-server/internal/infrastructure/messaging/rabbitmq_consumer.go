// Файл document-query-server/internal/infrastructure/messaging/rabbitmq_consumer.go содержит реализацию пакета messaging.
package messaging
...
// DocumentEvent represents a document domain event from RabbitMQ
...
// DocumentEventHandler processes received document events
type DocumentEventHandler interface {
	HandleDocumentCreated(ctx context.Context, event DocumentEvent) error
	HandleDocumentSent(ctx context.Context, event DocumentEvent) error
	HandleDocumentApproved(ctx context.Context, event DocumentEvent) error
	HandleDocumentRejected(ctx context.Context, event DocumentEvent) error
	HandleDocumentArchived(ctx context.Context, event DocumentEvent) error
}

// RabbitMQConsumer consumes document events from RabbitMQ
type RabbitMQDocumentConsumer struct {
	...
	validator *DocumentEventValidator
	closeMu   sync.Mutex
	closed    bool
...
// NewRabbitMQDocumentConsumer creates a new RabbitMQ consumer for document events
func NewRabbitMQDocumentConsumer(
	url, queueName, exchange string,
	logger *zap.Logger,
	handler DocumentEventHandler,
) (*RabbitMQDocumentConsumer, error) {
	...
	// Bind queue to exchange with routing keys for all document events
	routingKeys := []string{
		"document.created",
		"document.sent",
		"document.approved",
	...
	consumer := &RabbitMQDocumentConsumer{
		conn:      conn,
		channel:   channel,
		queue:     q.Name,
		exchange:  exchange,
		logger:    logger,
		handler:   handler,
		validator: NewDocumentEventValidator(logger),
		processed: make(map[string]bool),
	}

	logger.Info("RabbitMQ document consumer initialized",
...
func (c *RabbitMQDocumentConsumer) Start(ctx context.Context) error {
	c.closeMu.Lock()
	if c.closed {
		c.closeMu.Unlock()
...
func (c *RabbitMQDocumentConsumer) startWithReconnect(ctx context.Context) {
	reconnectAttempt := 0
...
func (c *RabbitMQDocumentConsumer) startConsuming(ctx context.Context) error {
...
func (c *RabbitMQDocumentConsumer) calculateReconnectBackoff(attempt int) time.Duration {
...
func (c *RabbitMQDocumentConsumer) processMessages(ctx context.Context, msgs <-chan amqp.Delivery) {
	for {
		select {
		case <-ctx.Done():
...
func (c *RabbitMQDocumentConsumer) calculateBackoff(retryCount int) time.Duration {
	baseDelay := time.Second
...
func (c *RabbitMQDocumentConsumer) isEventProcessed(ctx context.Context, eventID string) (bool, error) {
	c.procMu.RLock()
...
func (c *RabbitMQDocumentConsumer) markEventProcessed(ctx context.Context, eventID, eventType string) error {
	c.procMu.Lock()
...
func (c *RabbitMQDocumentConsumer) handleMessage(ctx context.Context, msg amqp.Delivery) error {
	...
	// Validate event
	validationErrors := c.validator.ValidateDocumentEvent(event)
	...
	case "document.created":
		processErr = c.handler.HandleDocumentCreated(ctx, event)
	case "document.sent":
		processErr = c.handler.HandleDocumentSent(ctx, event)
	case "document.approved":
		processErr = c.handler.HandleDocumentApproved(ctx, event)
	case "document.rejected":
		processErr = c.handler.HandleDocumentRejected(ctx, event)
	case "document.archived":
		processErr = c.handler.HandleDocumentArchived(ctx, event)
	default:
...
func (c *RabbitMQDocumentConsumer) Close() error {
	c.closeMu.Lock()
