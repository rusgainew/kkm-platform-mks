...
func Load() *Config {
	cfg := &Config{
		...
		DatabaseDriver:  getEnv("FOREIGN_COMPANY_QUERY_DB_DRIVER", "postgres"),
		DatabaseURL:     getEnv("FOREIGN_COMPANY_QUERY_DB_URL", ""),
...
func (c *Config) Validate() error {
	...
	if c.DatabaseURL == "" {
		return errors.New("FOREIGN_COMPANY_QUERY_DB_URL is required")
	}
	if c.DatabaseDriver == "" {
		return errors.New("FOREIGN_COMPANY_QUERY_DB_DRIVER is required")
	}
