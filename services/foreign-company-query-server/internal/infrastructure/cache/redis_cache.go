// Файл foreign-company-query-server/internal/infrastructure/cache/redis_cache.go содержит реализацию пакета cache.
package cache
...
func GenerateCacheKey(service, method string, params ...interface{}) string {
	return fmt.Sprintf("%s:%s:%v", service, method, params)
}
