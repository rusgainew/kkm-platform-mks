// Файл foreign-company-query-server/cmd/main.go содержит реализацию пакета main.
package main
...
import (
	...
	"github.com/rusgainew/kkm-project-mks/foreign-company-query-server/internal/infrastructure/config"
	apphealth "github.com/rusgainew/kkm-project-mks/foreign-company-query-server/internal/infrastructure/health"
	"github.com/rusgainew/kkm-project-mks/foreign-company-query-server/internal/infrastructure/middleware"
	"github.com/rusgainew/kkm-project-mks/foreign-company-query-server/internal/infrastructure/observability"
	"github.com/rusgainew/kkm-project-mks/foreign-company-query-server/internal/infrastructure/repository"
	"github.com/rusgainew/kkm-project-mks/foreign-company-query-server/internal/interfaces/grpc/handlers"
...
func main() {
	...
	tp, err := observability.InitTracer("foreign-company-query-service", cfg.JaegerEndpoint)
	if err != nil {
	...
	// Create in-memory repository (data will be populated via RabbitMQ events)
	repo := repository.NewInMemoryForeignCompanyRepository(logger)
	logger.Info("In-memory repository initialized")

	// Initialize metrics
	metrics := observability.NewMetricsCollector("foreign_company_query", "service")
	...
	healthServer.SetServingStatus("api.ForeignCompanyQueryService", grpc_health_v1.HealthCheckResponse_SERVING)

	// Register handler with repository, metrics, and cache
	handler := handlers.NewForeignCompanyQueryHandler(logger, repo, metrics, redisCache)
	pb.RegisterForeignCompanyQueryServiceServer(grpcServer, handler)

	// Initialize health check manager
	healthManager := apphealth.NewManager(logger)

	// Get the concrete repository type for health checks
	inMemoryRepo, ok := repo.(*repository.InMemoryForeignCompanyRepository)
	if ok {
	...
	logger.Info("Foreign Company Query Server starting", zap.Int("grpc_port", cfg.GRPCPort))
	...
	logger.Info("Foreign Company Query Server stopped")
}
