-- Создание таблицы для иностранных компаний
...
CREATE INDEX idx_foreign_companies_pin ON foreign_companies(pin);
CREATE INDEX idx_foreign_companies_full_name ON foreign_companies USING gin(to_tsvector('english', full_name));
CREATE INDEX idx_foreign_companies_country ON foreign_companies(country_code);
CREATE INDEX idx_foreign_companies_is_active ON foreign_companies(is_active);
CREATE INDEX idx_foreign_companies_created_at ON foreign_companies(created_at DESC);

-- Уникальный индекс для PIN (только среди активных компаний)
CREATE UNIQUE INDEX idx_foreign_companies_pin_unique ON foreign_companies(LOWER(pin)) WHERE is_active = true;

-- Комментарии к таблице
COMMENT ON TABLE foreign_companies IS 'Справочник иностранных компаний-контрагентов';
